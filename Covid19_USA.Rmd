...
Unfortunately, a lot of varibles have too many NA values. Some of them even over 50%. So here the first thing is we need to deal with the NA values
...
Firstly, I will deal with the missing values of deathConfirmed and deathProbable. These 2 variables mean the total fatalities. But they have the significant NA values. So we must deal with it carefully Here, firstly, I will find the patterns of these missing values. I consider 2 perspectives: 1 region 2 time

State-wise Pattern: We'll summarize the number of missing values for each state. Time-wise Pattern: We'll summarize the number of missing values over time

##state wise pattern analysis here we create a bar plot with same height, each bar shows the percentage of confirmed and probable missing values. And above the bar, we show the numbers of total missing values.
...
# Plotting
ggplot(state_missing_percentage, aes(x = state, y = 100)) +  # 100% bar height
  geom_bar(aes(fill = "Confirmed Percentage"), stat = "identity", width = 0.7) +
  geom_bar(aes(y = Probable_Percentage, fill = "Probable Percentage"), stat = "identity", width = 0.9) +
...
Not good enough, because the boundries of each bar is not clear. It influences us too much. Now I change the boundries to grey. And I need to make the canvas bigger to see everything clear
